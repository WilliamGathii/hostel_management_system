...
| ------ | --------------------------------------- | -------------------------------------------- | ------------------------------------------------- | -------------------------------- | ---------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------- |
| `POST` | `/api/v1/auth/login`                    | Start a normal or required-change login.     | Public                                            | `email`, `password`              | Normal `user`, `token`; or restricted password-change data | Account must be active. Admin-created Students must change their temporary password first. | `200`, `400`, `401`, `403`, `422`, `500` |
| `POST` | `/api/v1/auth/change-required-password` | Replace an Admin-created temporary password. | Restricted Student password-change session        | `newPassword`, `confirmPassword` | `passwordChanged`                                          | Restricted token must be valid. Passwords must match and differ from the temporary value.  | `200`, `401`, `422`, `500`               |
| `POST` | `/api/v1/auth/logout`                   | End the client session.                      | Student, Admin, Maintenance Staff, Security Staff | None                             | `message`                                                  | Normal access token must be valid.                                                         | `200`, `401`, `500`                      |
| `GET`  | `/api/v1/auth/me`                       | View the current authenticated user.         | Student, Admin, Maintenance Staff, Security Staff | None                             | `user`, `profile`                                          | Normal access token and credential version must be valid.                                  | `200`, `401`, `404`, `500`               |
...
| `POST`   | `/api/v1/students`                   | Create a Student account.                         | Admin         | `full_name`, `email`, `student_number`, `password`; optional profile fields             | `student`                | Email and Student number must be unique. The temporary password must be changed at first login.               | `201`, `400`, `401`, `403`, `409`, `422`, `500`        |
...
| `DELETE` | `/api/v1/students/:studentId`        | Delete an unused Student account.                 | Admin         | `studentId` path value                                                                  | Deleted Student identity | Student must have no room, maintenance, visitor, or payment history. Use inactive status when history exists. | `200`, `401`, `403`, `404`, `409`, `422`, `500`        |
...
| `GET`    | `/api/v1/rooms/floors`         | List floor summaries.       | Admin         | None                                                                                                           | `floors` with room, capacity, occupancy, and status counts        | Counts are calculated from saved rooms and active allocations.                                | `200`, `401`, `403`, `500`                      |
...
| `DELETE` | `/api/v1/rooms/:roomId`        | Delete an unused room.      | Admin         | `roomId` path value                                                                                            | Deleted room identity                                             | Room must have no allocation or maintenance history. Use inactive status when history exists. | `200`, `401`, `403`, `404`, `409`, `500`        |
